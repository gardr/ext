...
var bootStrap = function (options) {
    ...
    function resizeHandler(name) {
        var size = childrenSize(gardr.container);
        pluginApi.trigger('banner:' + name, size);
        com.rendered(size);
    }

    eventListener.add(doc, 'readystatechange', function readyStateChangeHandler() {
        if (/interactive|complete/.test(doc.readyState)) {
            resizeHandler(doc.readyState);
        }
    });

    // Faster in some IE versions
    eventListener.add(doc, 'DOMContentLoaded', function domContentLoadedHandler() {
        resizeHandler('domcontentloaded');
    });

    eventListener.add(global, 'load', function onloadDelayedHandler() {
        resizeHandler('rendered');
    });
